...
|---|---|---|---|
| TC full | `paper/tc/versions/v6/unimind_tc_v6.pdf` | 12 | current snapshot (v6 honesty pass) |
| TC compact (CAL submission) | `paper/tc/versions/v6/unimind_tc_compact6_v6.pdf` | 7 | current snapshot |
| Full technical report | `paper/unimind_paper_v2.6.pdf` | 41 | current snapshot |
...
1. **Temporal validity**: day-over-day calibration drift (D0→D1: ρ=0.579, τ=0.431, top-10
   Jaccard 0.25); stale top-3 pins are 2.1× worse than fresh within 24 h.
2. **Adaptive refresh**: a Jaccard<0.5 / ΔC>0.005 trigger detects staleness at ~0.04 ms/score;
   threshold sweeps over τ_J/τ_C reveal the refresh-vs-fidelity Pareto knee.
3. **Refresh dominates placement**: fresh pins lift both arms +33/37 pp on the 6-qubit suite;
   pinned vs free is statistically indistinguishable at n=30/arm (70.0 vs 76.7 %, p=0.56,
   Wilson intervals overlap [52.1,83.3]/[59.1,88.2], MDE≈26 pp, TOST not conclusive).
...
| path | contents |
|---|---|
| `data/drift/calib_2026-{08-29,08-30,08-31}.json` | D0 / D1 / D2 processed snapshots (marrakesh) |
| `data/calib_snapshots/ibm_marrakesh/` | raw per-pull snapshots of the 156-qubit device |
| `data/calib_snapshots/ibm_fez/` | raw 5-min telemetry pulls of the sibling device |
| `data/calib_snapshots/ibm_kingston/` | raw 15-min telemetry pulls of the third device |
