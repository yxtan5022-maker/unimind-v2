...
The calibration state that dominates this era's error budget is itself unstable: noise-aware transpilation folds day-specific device error into routing, yet large-device evidence~\cite{huowei2025} (five 127-qubit machines, 16 algorithms) finds that noise-aware transpilation concentrates work on a small qubit subset---increasing output-error variability---that random mapping matches its average fidelity, and that once-compiled circuits can be reused across calibration cycles at similar average fidelity. Our day-scale measurements qualify that average-fidelity conclusion on the tail: a $2.1\times$ stale-pin worst case and $0.25$ top-10 turnover are exactly the ``reuse breaks'' cases that quantity hides, motivating the refresh-triggered layout policy of Section~\ref{sec:adaptive}.
...
The v1.7 submission reported that the bare Unibit angle encoding misses the $|\langle Z\rangle|$ tolerance of $0.05$ on \texttt{ibm\_marrakesh}\footnote{Verified against the live IBM Quantum fleet on 2026-09-01 (device\_identity.py, notes/device\_identity.md): \texttt{ibm\_marrakesh} is still in the fleet and operational (156 qubits), so the D0--D2 results remain attributable to it, and \texttt{ibm\_fez}, the same-generation 156-qubit heavy-hex sibling used for the continuous telemetry, is a sibling device, not a successor.} (maximum deviation $0.392$) and that readout twirling recovers $37\%$. That experiment used \emph{free} transpiler placement (optimization level~1, no \texttt{initial\_layout}), so the physical qubit executing each circuit was uncontrolled and unrecorded. We first show that this confound, not the encoding itself, dominated the observed distortion.
...
\paragraph{T7: Calibration cadence and aliasing in the refresh analysis.} The D0--D2 snapshots of Section~\ref{sec:adaptive} are captured at the device's calibration-cycle cadence (one snapshot per update), and the refresh trigger is rate-limited to one re-fetch per cycle, so sub-cycle dynamics are not sampled; three cycles over roughly two days cannot resolve weekly or monthly maintenance cycles that shift the score surface in ways these points cannot see. The continuous 5-minute telemetry on the sibling device \texttt{ibm\_fez} (verified operational via device identity check, Section~\ref{sec:qpu}) postdates D0--D2 and is the instrument designed to close this gap; the drift statistics reported here are therefore day-scale bounds, not a full-cycle spectrum.
...
\bibitem{huowei2025}
Y.~Huo, J.~Wei, C.~Kverne, M.~Akewar, J.~Bhimani, and T.~Patel,
``Revisiting noise-adaptive transpilation in quantum computing: how much impact does it have?''
in \textit{Proc.\ IEEE/ACM Int.\ Conf.\ Computer-Aided Design (ICCAD)},
Munich, Germany, 2025, pp.~1--9; also \emph{arXiv:2507.01195}.
