...
6-qubit angle suite. The pinned vs.\ free difference is statistically indistinguishable under this
sample ($70.0\%$ vs.\ $76.7\%$, $p{=}0.56$; Wilson 95\% intervals $[52.1,83.3]$ and $[59.1,88.2]$
overlap, and the $\approx26$~pp minimum detectable effect at $n{=}30$/arm exceeds the measured
$6.7$~pp gap), and a TOST equivalence test at $\delta{=}15$~pp is not conclusive; we claim
indistinguishability under this sample, never equivalence. A negative result completes the picture:
re-fitting the 7-dim layout utility $J(G)$ on real device labels collapses its out-of-sample Spearman
from $\rho{=}0.818$ to $0.13$
...
topology, 176 couplers).%
\footnote{Verified against the live IBM Quantum fleet on 2026-09-01 (device\_identity.py, notes/device\_identity.md):
\texttt{ibm\_marrakesh} is still in the fleet and operational (156 qubits), so the D0--D2 results remain attributable
to it, and \texttt{ibm\_fez}, the same-generation 156-qubit heavy-hex sibling used for the continuous telemetry, is a
sibling device, not a successor.} We captured three full calibration snapshots---D0, D1, D2---at roughly 24-hour and 10-hour
...
problem''. Complementarily, recent large-device evidence~\cite{huowei2025} (five 127-qubit machines, 16 algorithms)
finds that noise-aware transpilation actually concentrates work on a small qubit subset---increasing output-error
variability---that random mapping matches its average fidelity, and that once-compiled circuits can be reused across
calibration cycles at similar average fidelity. Our day-scale measurements qualify that average-fidelity conclusion
on the tail: a $2.1\times$ stale-pin worst case and $0.25$ top-10 turnover are exactly the ``reuse breaks'' cases
that quantity hides, and the refresh trigger of \S\ref{sec:drift} is the policy that polices them.
...
device, one topology family (heavy-hex); the value is the protocol, not the constants. \textbf{Calibration cadence
and aliasing:} snapshots are captured at the device's calibration-cycle cadence (one snapshot per update) and the
refresh trigger is rate-limited to one re-fetch per cycle, so sub-cycle dynamics are not sampled; D0--D2 span three
cycles over roughly two days, and a daily cadence could alias longer (weekly/monthly) maintenance cycles that shift
the score surface in ways these three points cannot resolve. The continuous 5-minute telemetry on the sibling
\texttt{ibm\_fez} postdates D0--D2 and is the instrument designed to close this gap; the drift statistics here are
therefore day-scale bounds, not a full-cycle spectrum. \textbf{Placement null power:} $n{=}30$/arm can detect only
large effects; we report point estimates and CIs rather than asserting equivalence. \textbf{Selection fairness:} the greedy chain maximizes $C(q)$-locality and at large $k$ is forced to
...
\bibitem{huowei2025} Y.~Huo, J.~Wei, C.~Kverne, M.~Akewar, J.~Bhimani, and T.~Patel, ``Revisiting noise-adaptive transpilation in quantum computing: how much impact does it have?'' in \emph{Proc. IEEE/ACM Int.\ Conf.\ Computer-Aided Design (ICCAD)}, Munich, Germany, 2025, pp.~1--9; also \emph{arXiv:2507.01195}.
