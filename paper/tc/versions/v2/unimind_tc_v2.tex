...
% ---------------------------------------------------------------- J(G)
\section{The $J(G)$ Utility: Proxy Fit vs.\ Real Labels}
\label{sec:jreal}
...
\label{sec:sweep}
Using the D0$\to$D1 transition we simulate each policy offline. For every $(\tau_J, \tau_C)$ pair the simulation
tracks (i)~how many refreshes fire, (ii)~how many are \emph{necessary} (the true top-10 or best qubit changed),
and (iii)~how many \emph{missed drifts} occur (the trigger did not fire but the ranking shifted). The metrics are
...
\end{equation}
where pass$_i$ denotes whether parameterization $i$ meets the $0.05$ tolerance. The resulting Pareto curve
(Section~\ref{sec:e2e}) shows that the measured trigger ($\tau_J{=}0.5$, $\tau_C{=}0.005$) sits near the knee of
the refresh-rate-vs.\-fidelity tradeoff; relaxing $\tau_J$ below $0.3$ or tightening $\tau_C$ above $0.01$
moves the operating point to an inefficient region.
