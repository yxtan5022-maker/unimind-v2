...
(\texttt{ibm\_marrakesh}) across three primary daily snapshots (D0--D2; 2026-08-29 to 08-31) and,
to bound the validity window, 34 hourly calibration snapshots (2026-09-01 to 09-02, three backends).
We build \textbf{UniMind}, a middleware that scores, places, executes, and refreshes calibration
data in a closed loop. Four findings are established:
\textbf{(1)~Calibration validity is temporally bounded and small.} Day-over-day rank correlation is only
$\rho{=}0.579$, top-10 overlap is $0.25$, and a stale top-3 pin is $1.94$--$2.1\times$ worse than a
fresh selection across D0$\to$D1/D2 (24--34~h). Hourly telemetry shows the same drift mechanism---tiny
top-3 margins ($\sim10^{-3}$) versus median $|\Delta C|=0.0127$---explaining daily turnover while
margins remain small and drift over D1$\to$D2 is quiescent.
...
$T_{\mathrm{valid}}$ on a 156-qubit device across three anchored daily snapshots (plus 34 hourly
snapshots for the drift mechanism), design an adaptive refresh trigger that detects staleness at
...
All quantitative claims below are measured on \texttt{ibm\_marrakesh} (156 qubits, heavy-hex);
three daily snapshots (D0--D2) anchor every QPU job, and 34 hourly snapshots (2026-09-01 to 09-02,
\texttt{marrakesh} + \texttt{kingston} + \texttt{fez}) bound the drift mechanism without additional
...
(stale top-3 is $1.94$--$2.1\times$ worse than a fresh selection across D0$\to$D1/D2 (24--34~h),
Table~\ref{tab:stale}), formalize
the Calibration Validity Horizon $T_{\mathrm{valid}}$ (Def.~\ref{def:tvalid}) as a
\emph{lower-bound} design primitive, and explain turnover via ranking margins
($M_3\sim10^{-3}$ vs.\ median $|\Delta C|=0.0127$, \S\ref{sec:drift}).
...
with three anchored QPU snapshots plus 34 hourly calibration snapshots we establish its lower bound and
mechanism on one device family (heavy-hex), but leave the full multi-day decay curve and cross-family
generalization to future work. The LLM-orchestration layer is a controlled-parameter reliability model
...
snapshot (Table~\ref{tab:snap}). Two snapshot families recur: the \emph{pinned} family
\texttt{calib\_2026-08-\{29,30,31\}.json}, and a legacy 08-23 capture used only for the placement
anchor of \S\ref{sec:placement}. Where ``stale'' pins are evaluated, the pins are always taken from
D0 while execution happens on D1/D2-era device state.

To bound the drift mechanism without additional QPU cost we also analyze 34 hourly calibration
snapshots from 2026-09-01 01:03 to 2026-09-02 02:55 across \texttt{ibm\_marrakesh} (30 captures),
\texttt{ibm\_kingston} and \texttt{ibm\_fez}. These are calibration-only (no jobs) and are used in
\S\ref{sec:drift} to quantify ranking-margin turnover and hourly stability; they do not change any
QPU job label.

\begin{table}[t]
\caption{Snapshot timeline. Pins for each experiment family reference the stated snapshot; execution times are the job
queue windows recorded in the data files. Extended hourly telemetry (2026-09-01 to 09-02, 34 captures) is calibration-only.}
\label{tab:snap}
\centering
\footnotesize
...
hourly & 2026-09-01--02 & 34$\times$ & drift mechanism only\\
\bottomrule
\end{tabular}
\end{table}
...
\label{sec:drift}
This section measures how fast the score goes stale. D0 (2026-08-29~21:26) and D1 (2026-08-30~21:29) are the
primary 156-qubit snapshots anchoring every QPU job; D2 (2026-08-31~07:46) is the refresh arm. To expose the
\emph{mechanism} without additional QPU cost we also use 34 hourly calibration snapshots (2026-09-01 to 09-02,
\texttt{marrakesh} + \texttt{kingston} + \texttt{fez}) from \texttt{analysis/results/refresh\_policy\_analysis.json}
and \texttt{ranking\_margin\_analysis.json}.
...
(only four of D0's best ten survive; D0$\to$D2 Jaccard $0.11$, D1$\to$D2 $0.176$). The best qubit turns over:
\q{8} ($C{=}0.0039$, rank 1) rises to $0.0095$ (rank 6) while \q{19} ($0.0066$) becomes best. Per-qubit spread is
extreme (Fig.~\ref{fig:driftq}): \q{37} swings $0.099{\to}0.740$ ($\max|\Delta C|{=}0.641$), \q{53} improves sixfold
($0.072{\to}0.011$); median $|\Delta C|$ is $0.0127$, $p90$ is $0.123$, and $72.4\%$ of qubits move by more than
$0.005$ ($53.9\%$ by $0.01$, $41.0\%$ by $0.02$). On D2 the top of the ranking is unchanged (best remains \q{19}
with the same top-3 values), consistent with a regime dominated by daily calibration cycles. Table~\ref{tab:paperdrift}
...
\emph{Hourly mechanism:} The fragility is not accidental. On D0 the top-3 margins are $M_3\sim0.0015$ (gaps
between \q{8}/\q{120}/\q{153}), an order of magnitude smaller than median drift $0.0127$. Hence a typical daily
$|\Delta C|$ reorders the top-10. On D1 the margins remain small but drift over the next 10~h (D1$\to$D2) is
quiescent (best-qubit $\Delta C$ stable, Jaccard $0.176$), so D1$\to$D2 does not fire the trigger. The hourly
telemetry confirms: D0$\to$D1 fires (Jaccard $0.25<0.5$, $\Delta C{=}0.0056>0.005$) while D1$\to$D2 would not,
exactly as the two-regime hypothesis predicts (\texttt{ranking\_margin\_analysis.json}: top-3 gap D0$\to$D1
$1.46\times10^{-3}$, D1$\to$D2 gap $0$ within measurement noise).
...
$0.0066$). The same mean comparison on D0$\to$D2 is $2.0\times$ ($0.0153$ vs.\ $0.0076$); the extreme
worst-vs-best on D0$\to$D2 is $3.3\times$ ($0.0209$ vs.\ $0.0063$) and is not quoted as the portable effect.
We therefore quote the conservative mean/worst range $1.94$--$2.1\times$ across D0$\to$D1/D2 (24--34~h) as the
portable staleness cost. The trigger of
\S\ref{sec:trigger} fires on D0$\to$D1 (Jaccard $0.25<0.5$; stored-best $\Delta C{=}0.0056>0.005$) and would not
fire D1$\to$D2 (top-3 unchanged, Jaccard $0.176$), matching the regime hypothesis. Because re-selection is
...
For the $k{=}6$ chain and tolerance $\epsilon{=}0.05$, the selected pins remain within tolerance on \emph{this
device} at $t{+}10$~h (D1$\to$D2 top-3 unchanged, $T_{\mathrm{valid}}\gtrsim10$~h, calibration-only) but the cost
of a one-day-old ranking already reaches $1.94$--$2.1\times$ (D0$\to$D1/D2, QPU-anchored, $T_{\mathrm{valid}}\lesssim
24$~h). $T_{\mathrm{valid}}$ is therefore \emph{hours, not days, on this device at this $k$}---small enough that
...
tight because margins $M_3$ are $\mathcal{O}(10^{-3})$ while drift is $\mathcal{O}(10^{-2})$.
...
($\rho{=}0.58$, top-10 Jaccard $0.25$, stale worst pin $1.94$--$2.1\times$ worse, hourly margins
$M_3\sim10^{-3}$ vs.\ drift $0.0127$) that makes selection age the first-order risk, and refresh is cheap
...
stale snapshot invalidates selection & \S\ref{sec:drift}: stale best pin $1.94$--$2.1\times$ worse & trigger-based refresh; snapshot age\\
...
device, one heavy-hex family; the value is the protocol, not the constants. Hourly calibration-only telemetry
(34 captures, 3 backends) bounds the \emph{mechanism} (margins vs.\ drift) but QPU validation across families
remains future work. \textbf{Placement null power:} $n{=}30$/arm can detect only large effects (MDE
...
a 156-qubit device (anchored D0--D2, plus 34 hourly calibration snapshots for the drift mechanism): a
score-only placement pipeline with measured scope, daily drift dynamics with a working adaptive trigger at the
Pareto knee, isolation of the proxy-trained $J(G)$ collapse mechanism (OOS $\rho$ $0.818{\to}0.13$, feature
...
work: multi-day D2--D5 QPU accumulation, refresh-conditional placement ($C(q)$-vs-connectivity trade-off,
...
calib\_snapshots/ibm\_mar\-rakesh/2026-09-01*.json (34 hourly)\\
...
workload\_connectivity\_drift.json\\
\end{quote}
