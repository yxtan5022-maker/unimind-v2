...
% ---------------------------------------------------------------- Abstract
\begin{abstract}
Quantum-accelerated AI workloads must be scheduled onto devices whose calibration changes between runs.
This paper is a measurement study: we instrument a 156-qubit device across three daily snapshots
(D0--D2) and 34 hourly calibration snapshots (three backends) and build \textbf{UniMind}, a middleware
that scores, places, executes, and refreshes calibration data. Four findings: \textbf{(1)~Validity is
bounded and small} ($\rho{=}0.579$, top-10 Jaccard $0.25$, stale top-3 $1.94$--$2.1\times$ worse
across D0$\to$D1/D2 (24--34~h); hourly margins $M_3\sim10^{-3}$ vs.\ median $|\Delta C|=0.0127$). We formalize the
\emph{Calibration Validity Horizon} $T_{\mathrm{valid}}$ as a lower-bound primitive. \textbf{(2)~Adaptive
refresh at near-zero cost} (Jaccard $<0.5$ or $\Delta C{>}0.005$, $\sim0.04$~ms) fires once on D0$\to$D1
and restores $+33$~pp while periodic-$6$h fires $5\times$; 144-threshold sweep places the default at
the Pareto knee. \textbf{(3)~Refresh dominates placement} at $k{=}6$ ($+33/37$~pp, Fisher $p{<}0.02$;
pinned vs.\ free $70.0\%$ vs.\ $76.7\%$, $p{=}0.77$, MDE $\approx32$~pp, bounded null).
\textbf{(4)~Proxy models do not transfer}: $J(G)$ size-OOS $\rho$ $0.818{\to}0.13$ ($p{=}0.36$); ablation
and variance crossover at $k\approx100$ isolate the mechanism. We give the model, numbers, trigger, and
negative results.
\end{abstract}
...
This paper addresses the temporal dimension. We define the \textbf{Calibration Validity
Horizon} $T_{\mathrm{valid}}$ and measure its \emph{lower bound} on a 156-qubit device across three anchored daily
snapshots plus 34 hourly calibration snapshots for the mechanism, design an adaptive trigger at near-zero cost, and
show refresh---not placement---is where reliability engineering should invest at the measured scale.

This paper makes four contributions, all anchored to three daily \texttt{ibm\_marrakesh} snapshots
(D0--D2) plus 34 hourly calibration snapshots (three backends, calibration-only) for the drift mechanism:
\begin{enumerate}[leftmargin=1.8em,itemsep=1pt,topsep=2pt]
\item \textbf{Temporal validity as a lower bound.} Drift $\rho{=}0.579$, $\tau{=}0.431$, top-10 Jaccard
$0.25$; stale top-3 $1.94$--$2.1\times$ worse in $24$--$34$~h; $T_{\mathrm{valid}}\gtrsim10$~h,
$\lesssim24$~h on this device at $k{=}6$; margins $M_3\sim10^{-3}$ explain turnover
(\S\ref{sec:drift}).
\item \textbf{Adaptive refresh with explicit baselines.} Trigger Jaccard $<0.5$ or $\Delta C{>}0.005$
($\sim0.04$~ms), 144-threshold sweep, static/periodic/adaptive simulation: default fires once for
$+33$~pp; periodic-$6$h fires $5\times$ for same drift (\S\ref{sec:policies}).
\item \textbf{Bounded placement.} $J(G)$ size-OOS $\rho$ $0.818{\to}0.13$ ($p{=}0.36$); at $k{=}6$ pinned vs.\
free is a bounded null ($70.0\%$ vs.\ $76.7\%$, Fisher $p{=}0.77$, MDE $\approx32$~pp). Ablation +
connectivity crossover at $k\approx100$ explain the mechanism (\S\ref{sec:jreal},\S\ref{sec:e2e}).
\end{enumerate}

\noindent\textbf{Scope.} Measurement study, not quantum advantage or production runtime. $T_{\mathrm{valid}}$ is a
lower-bound primitive on one heavy-hex family; hourly telemetry bounds the mechanism but cross-family QPU
validation is future work. Every claim traces to a job/snapshot (\S\ref{sec:data}); failed $\rho{=}1.000\to0.771$
is reported. The LLM layer is a separable controlled-parameter model (App.~D), not a hardware claim.
...
is $2.1\times$ worse than the adaptable choice on D0$\to$D1 ($0.0139$ vs.\ $0.0066$); D0$\to$D2 mean is $2.0\times$
($0.0153$ vs.\ $0.0076$), range $1.94$--$2.1\times$ across 24--34~h. The trigger of \S\ref{sec:trigger} fires on
D0$\to$D1 (Jaccard $0.25<0.5$; stored-best $\Delta C{=}0.0056>0.005$). Because re-selection is sub-millisecond,
...
95\% CI; Fisher exact $p$; MDE $\approx32$~pp at $80\%$ power.}
\label{tab:e2e}
\centering
\footnotesize
\setlength{\tabcolsep}{4pt}
...
Refresh $p$ (Fisher) & $0.019$ & $0.008$ & ---\\
\bottomrule
\end{tabular}
\end{table}

Two effects separate cleanly. First, \emph{temporality is significant}: refreshing pins lifts \emph{both} arms by
$+33/37$~pp (Fisher $p{=}0.019$/$0.008$) including the free arm. Second, \emph{placement is a bounded null}
($-3.3$/$-6.7$~pp, Fisher $p{=}1.00$/$0.77$, MDE $\approx32$~pp), so only large advantages are excluded at $k{=}6$.
This is the honest scope: placement is decisive for extremes (\q{98} vs.\ \q{37}: $0.028$ vs.\ $0.213$) and sets the
floor at $k\ge9$ under stale pins, but average benefit at small $k$ is within temporal noise.
The full pipeline (App.~D) measured $54/54$ vs.\ $47/54$ ($87.0\%$, $z{=}2.74$, $p{=}0.003$) vs.\ model's $97.3\%$;
the earlier $54$-sample frozen $74.1\%$ vs.\ $66.7\%$ is reported as a failed claim.
...
$1.94$--$2.1\times$ worse across D0$\to$D1/D2 (24--34~h)) that makes selection age the first-order risk, and
refresh is cheap; (iii)~simulator-proxy
