...
{\normalsize\rmfamily Version 2.1 (August 23, 2026)\par}
...
\paragraph{Implementation divergence: gate order in Algorithm~\ref{alg:mapping}.} We report a spec--code discrepancy discovered during a post-submission audit. As written, Algorithm~\ref{alg:mapping} applies the conditional $X$ gate \emph{before} the rotation ($R_y(\theta_i)X|0\rangle = \cos(\theta/2)|1\rangle + \sin(\theta/2)|0\rangle$), which yields $P(b'_i = 1) = 1 - w_i$ for positions with $b_i = 1$ --- inverting, rather than realizing, the intended weight-to-probability map on those qubits. All verification experiments reported in this paper (Sections~\ref{sec:math_verify}, \ref{sec:noise}, and \ref{sec:qpu}) prepare their target states from the analytic $\theta_i$ directly, without the conditional $X$, and are therefore unaffected; the divergence concerns only the full-pipeline path from raw bit sequences through \texttt{qunibit}. The fix is a one-line reordering (apply $R_y$ first, or drop the conditional $X$ entirely since $R_y(2\arcsin\sqrt{w})$ already realizes the target distribution); it is scheduled for the next code release together with a regression test asserting $P(1) = w_i$ end-to-end.
