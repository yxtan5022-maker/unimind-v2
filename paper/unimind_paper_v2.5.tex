...
\paragraph{Limitations (Stage~3).} $n{=}23$ synthetic-expanded (14 FakeMarrakesh points, same 2026-08-29 snapshot; $y{=}$est\_2q proxy, not QPU-measured). v3's $n{=}9$ overfit is resolved (LOO $\rho$ $0.667{\to}0.953$, $oob\_clipped$ eliminated, $n$/dims $9/7{\to}23/7$), and the size-OOS ordering transfer ($\rho=0.818$, $k{\ge}9$) supports decision use for $k\le 18$; hardware confirmation for $k{>}8$ remains pending and weights should not be interpreted individually (stability $L_1{=}0.84$).
